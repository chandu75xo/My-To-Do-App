...
export default function TaskCard({ task, onToggle, onDelete, onToggleImportant, onEdit }) {
    ...
    `}>
      ...
      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onToggle(task.id)}>
        ...
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          {dueLabel && (
            <span className={`text-xs flex items-center gap-1 ${overdue ? 'text-red-500 dark:text-red-400 font-medium' : 'text-gray-400 dark:text-gray-500'}`}>
              <svg className="w-3 h-3" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                <rect x="2" y="3" width="12" height="11" rx="2"/><path d="M5 1v2M11 1v2M2 7h12" strokeLinecap="round"/>
              </svg>
      ...
      <div className="flex items-center gap-0.5 flex-shrink-0">
        ...
        <button onClick={() => onToggleImportant(task.id)}
          ...
          title={task.important ? 'Important on' : 'Mark important'}>
