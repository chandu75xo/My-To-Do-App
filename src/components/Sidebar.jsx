export default function Sidebar({
  ...
  onTestPush, testStatus, onSignOut, onOpenSettings
}) {
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 z-40 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-3">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={onTestPush}
                      ...
                      {testStatus === 'sending' ? 'Sending…' :
                       testStatus === 'sent'    ? '✓ Server sent!' :
                       testStatus === 'error'   ? '✗ Failed' : 'Push test'}
                    </button>
                  <button onClick={() => { onDisableNotifications(); onClose() }}
                    className="w-full py-2 rounded-xl text-xs font-medium bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40 text-red-600 dark:text-red-400 hover:bg-red-100 transition-all">
          ...
          <button className="w-full flex items-center gap-3 px-5 py-2.5 text-left text-gray-400 dark:text-gray-600 cursor-default">
            ...
            <span className="text-xs italic text-gray-300 dark:text-gray-600">coming in v4</span>
          </button>
          <button className="w-full flex items-center gap-3 px-5 py-2.5 text-left text-gray-400 dark:text-gray-600 cursor-default">
            <span className="text-base w-5 text-center">✦</span>
        ...
        <div className="p-5 border-t border-gray-100 dark:border-gray-800">
          <p className="text-xs text-gray-400 dark:text-gray-600 text-center">done. v3.0 — PWA</p>
        </div>
      </aside>
    </>
  )
